...
void dialogs_init(Layer *canvas_layer) { s_canvas_layer = canvas_layer; }
...
static void dialog_window_load(Window *window) {
    ...
    text_layer_set_text(s_dialog_text_layer,
                        s_dialog_message ? s_dialog_message : "Dialog");
    text_layer_set_text_alignment(s_dialog_text_layer, GTextAlignmentCenter);
    text_layer_set_font(s_dialog_text_layer,
...
void show_dialog(const char *message) {
    ...
    window_set_window_handlers(s_dialog_window,
                               (WindowHandlers){
...
void show_ko_dialog(const char *message) {
    ...
    s_dialog_timer =
        app_timer_register(2000, (AppTimerCallback)hide_dialog, NULL);
}
...
static void scroll_dialog_window_load(Window *window) {
    ...
    text_layer_set_text(s_scroll_text_layer,
                        s_dialog_message ? s_dialog_message : "");
    text_layer_set_font(s_scroll_text_layer,
                        fonts_get_system_font(FONT_KEY_GOTHIC_18));
    text_layer_set_text_alignment(s_scroll_text_layer, GTextAlignmentCenter);
    text_layer_set_overflow_mode(s_scroll_text_layer,
    ...
    GSize text_size = graphics_text_layout_get_content_size(
        s_dialog_message ? s_dialog_message : "",
        fonts_get_system_font(FONT_KEY_GOTHIC_18),
        GRect(0, 0, bounds.size.w - 4, 2000), GTextOverflowModeWordWrap,
        GTextAlignmentCenter);

    layer_set_bounds(text_layer_get_layer(s_scroll_text_layer),
                     GRect(0, 0, bounds.size.w, text_size.h + 10));
    scroll_layer_add_child(s_scroll_layer,
                           text_layer_get_layer(s_scroll_text_layer));
    scroll_layer_set_content_size(s_scroll_layer,
                                  GSize(bounds.size.w, text_size.h + 20));
...
void show_scroll_dialog(const char *message) {
    ...
    window_set_window_handlers(s_scroll_dialog_window,
                               (WindowHandlers){
...
static void gameover_window_load(Window *window) {
    ...
    snprintf(message, sizeof(message), "%s\n\nB: %d.%d  W: %d.%d", winner,
             black_total_10x / 10, black_total_10x % 10, white_total_10x / 10,
             white_total_10x % 10);
    ...
    text_layer_set_font(s_gameover_text_layer,
                        fonts_get_system_font(FONT_KEY_GOTHIC_18_BOLD));
...
void hide_gameover_dialog(void) {
    ...
    if (s_gameover_dismiss_callback)
        s_gameover_dismiss_callback();
    if (s_canvas_layer)
        layer_mark_dirty(s_canvas_layer);
}
...
static void error_window_load(Window *window) {
    ...
    text_layer_set_text(s_error_text_layer,
                        s_dialog_message ? s_dialog_message : "Error");
    text_layer_set_text_alignment(s_error_text_layer, GTextAlignmentCenter);
    text_layer_set_font(s_error_text_layer,
                        fonts_get_system_font(FONT_KEY_GOTHIC_18_BOLD));
...
static void error_click_config(void *context) {
    window_single_click_subscribe(BUTTON_ID_SELECT,
                                  (ClickHandler)hide_error_dialog);
    window_single_click_subscribe(BUTTON_ID_BACK,
                                  (ClickHandler)hide_error_dialog);
}
...
void show_error_dialog(const char *message) {
    ...
    window_set_window_handlers(s_error_dialog,
                               (WindowHandlers){
    ...
    s_error_timer =
        app_timer_register(1500, (AppTimerCallback)hide_error_dialog, NULL);
}
